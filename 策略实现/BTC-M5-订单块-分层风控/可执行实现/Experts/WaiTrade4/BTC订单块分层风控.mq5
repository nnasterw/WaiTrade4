...
#include "../../Include/WaiTrade2/Config.mqh"
#include "../../Include/WaiTrade2/Types.mqh"
#include "../../Include/WaiTrade2/Utils.mqh"
#include "../../Include/WaiTrade2/MarketState.mqh"
#include "../../Include/WaiTrade2/ScoreEngine.mqh"
#include "../../Include/WaiTrade2/DecayDetector.mqh"
#include "../../Include/WaiTrade2/OBDetector.mqh"
#include "../../Include/WaiTrade2/SignalEngine.mqh"
#include "../../Include/WaiTrade2/EntryEngine.mqh"
#include "../../Include/WaiTrade2/PositionManager.mqh"

input string InpWT4审计运行标识 = "";
int g_wt4_权益文件 = INVALID_HANDLE;
int g_wt4_风险文件 = INVALID_HANDLE;

bool WT4审计标识有效(const string 标识)
{
    if(StringLen(标识) < 8 || StringLen(标识) > 96) return false;
    for(int i = 0; i < StringLen(标识); i++)
    {
        ushort 字符 = StringGetCharacter(标识, i);
        if(!((字符 >= 'a' && 字符 <= 'z') || (字符 >= 'A' && 字符 <= 'Z') ||
             (字符 >= '0' && 字符 <= '9') || 字符 == '-' || 字符 == '_')) return false;
    }
    return true;
}
...
string WT4毫秒时间()
{
    ...
    datetime 秒 = (datetime)(tick.time_msc / 1000);
    return TimeToString(秒, TIME_DATE | TIME_SECONDS) + "." + StringFormat("%03d", (int)(tick.time_msc % 1000));
}

bool WT4初始化审计()
{
    if(InpWT4审计运行标识 == "") return true;
    if(!WT4审计标识有效(InpWT4审计运行标识))
    {
        Print("WT4审计运行标识无效");
        return false;
    }
    string 目录 = "wt4\\audit\\" + InpWT4审计运行标识;
    // FolderCreate 在目录已存在时也会失败；由后续独占 FileOpen 作为
    // 目录可写性和本轮审计原件未被覆盖的最终判据。
    FolderCreate("wt4");
    FolderCreate("wt4\\audit");
    FolderCreate(目录);
    if(FileIsExist(目录 + "\\equity.csv") || FileIsExist(目录 + "\\opening_risk.csv"))
    {
        Print("WT4审计原件已存在，拒绝覆盖");
        return false;
    }
    g_wt4_权益文件 = FileOpen(目录 + "\\equity.csv", FILE_WRITE | FILE_CSV | FILE_ANSI, ',');
    g_wt4_风险文件 = FileOpen(目录 + "\\opening_risk.csv", FILE_WRITE | FILE_CSV | FILE_ANSI, ',');
    if(g_wt4_权益文件 == INVALID_HANDLE || g_wt4_风险文件 == INVALID_HANDLE)
    {
        Print("WT4审计文件创建失败: ", GetLastError());
        return false;
    }
    FileWrite(g_wt4_权益文件, "time", "balance", "equity");
    FileWrite(g_wt4_风险文件, "deal_id", "time", "equity", "initial_risk", "open_initial_risk");
    FileFlush(g_wt4_权益文件);
    FileFlush(g_wt4_风险文件);
    return true;
}

void WT4记录权益()
{
    if(g_wt4_权益文件 == INVALID_HANDLE) return;
    string 时间 = WT4毫秒时间();
    if(时间 == "") return;
    FileWrite(g_wt4_权益文件, 时间,
              DoubleToString(AccountInfoDouble(ACCOUNT_BALANCE), 8),
              DoubleToString(AccountInfoDouble(ACCOUNT_EQUITY), 8));
    FileFlush(g_wt4_权益文件);
}

double WT4持仓初始风险(const ulong 持仓号, const double 当前价格, const double 手数)
{
    if(!PositionSelectByTicket(持仓号)) return -1.0;
    double 止损 = PositionGetDouble(POSITION_SL);
    double 最小跳动 = SymbolInfoDouble(_Symbol, SYMBOL_TRADE_TICK_SIZE);
    double 每跳价值 = SymbolInfoDouble(_Symbol, SYMBOL_TRADE_TICK_VALUE);
    if(止损 <= 0 || 最小跳动 <= 0 || 每跳价值 <= 0 || 当前价格 <= 0 || 手数 <= 0) return -1.0;
    return MathAbs(当前价格 - 止损) * 手数 * 每跳价值 / 最小跳动;
}
...
double WT4当前开放初始风险()
{
    ...
    {
        ulong 持仓号 = PositionGetTicket(i);
        if(持仓号 == 0 || !PositionSelectByTicket(持仓号)) continue;
        if(PositionGetString(POSITION_SYMBOL) != _Symbol ||
           PositionGetInteger(POSITION_MAGIC) != InpMagicNumber) continue;
        double 风险 = WT4持仓初始风险(持仓号, PositionGetDouble(POSITION_PRICE_OPEN), PositionGetDouble(POSITION_VOLUME));
        if(风险 < 0) return -1.0;
        合计 += 风险;
    }
    return 合计;
...
void WT4记录开仓风险(const MqlTradeTransaction &事务)
{
    if(g_wt4_风险文件 == INVALID_HANDLE || 事务.type != TRADE_TRANSACTION_DEAL_ADD || 事务.deal == 0) return;
    if(!HistoryDealSelect(事务.deal)) return;
    if(HistoryDealGetString(事务.deal, DEAL_SYMBOL) != _Symbol ||
       HistoryDealGetInteger(事务.deal, DEAL_MAGIC) != InpMagicNumber ||
       HistoryDealGetInteger(事务.deal, DEAL_ENTRY) != DEAL_ENTRY_IN) return;
    ulong 持仓号 = (ulong)HistoryDealGetInteger(事务.deal, DEAL_POSITION_ID);
    double 单笔风险 = WT4持仓初始风险(持仓号, HistoryDealGetDouble(事务.deal, DEAL_PRICE), HistoryDealGetDouble(事务.deal, DEAL_VOLUME));
    double 开放风险 = WT4当前开放初始风险();
    if(单笔风险 <= 0 || 开放风险 < 单笔风险)
    {
        Print("WT4审计拒绝缺失服务器止损或合约风险规格: deal=", 事务.deal);
        return;
    }
    datetime 秒 = (datetime)HistoryDealGetInteger(事务.deal, DEAL_TIME);
    FileWrite(g_wt4_风险文件, (long)事务.deal, TimeToString(秒, TIME_DATE | TIME_SECONDS),
              DoubleToString(AccountInfoDouble(ACCOUNT_EQUITY), 8),
              DoubleToString(单笔风险, 8), DoubleToString(开放风险, 8));
    FileFlush(g_wt4_风险文件);
}

void WT4关闭审计()
{
    if(g_wt4_权益文件 != INVALID_HANDLE) { FileClose(g_wt4_权益文件); g_wt4_权益文件 = INVALID_HANDLE; }
    if(g_wt4_风险文件 != INVALID_HANDLE) { FileClose(g_wt4_风险文件); g_wt4_风险文件 = INVALID_HANDLE; }
}
...
int OnInit()
{
    ...
    if(!WT4初始化审计()) return INIT_FAILED;
...
void OnTradeTransaction(const MqlTradeTransaction &事务, const MqlTradeRequest &请求, const MqlTradeResult &结果)
{
    WT4记录开仓风险(事务);
}
