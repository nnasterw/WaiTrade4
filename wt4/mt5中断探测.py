...
class 中断无污染结果:
    ...
    被中断时仍运行: bool
    未中断完成: bool
    被中断Wine服务进程号: tuple[int, ...]
    未中断Wine服务进程号: tuple[int, ...]
...
class 两实例中断探测器:
    ...
    def 执行(self, 启动宽限秒: float, 未中断超时秒: int, 第二实例启动间隔秒: float = 2) -> 中断无污染结果:
        ...
        while monotonic() < 截止:
            ...
            sleep(0.1)
        被中断时仍运行 = 甲.快照().状态 == "运行中"
        甲Wine服务 = 甲.认领自有Wine服务()
        # 不使用全局 wineserver/pkill；该调用只向甲创建的进程组发信号。
        甲.终止自有进程组()
        甲返回码 = 甲.等待(10)
        甲Wine服务 = tuple(sorted(set(甲Wine服务) | set(甲.终止自有Wine服务())))
        乙返回码 = 乙.等待(未中断超时秒)
        if 乙返回码 is None:
            乙.终止自有进程组()
            乙返回码 = 乙.等待(10)
        乙Wine服务 = 乙.终止自有Wine服务()
        return 中断无污染结果(
            甲返回码, 乙返回码, 甲.快照().状态 == "已退出", 被中断时仍运行, 乙返回码 == 0, 甲Wine服务, 乙Wine服务
        )
