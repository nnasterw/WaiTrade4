...
class _仅登录执行器适配器:
    """让中央编排器以常规 ``执行`` 协议调用仅登录实验。"""

    def __init__(self, 执行器: 单实例MT5探测执行器) -> None:
        self.执行器 = 执行器

    def 执行(self, 输入: 实验输入, 暂存目录: Path):
        return self.执行器.执行仅登录同步(输入, 暂存目录)
...
def main() -> None:
    ...
    参数.add_argument("--仅登录等待同步", action="store_true")
    实参 = 参数.parse_args()
    ...
    if 实参.仅登录等待同步:
        输入 = 实验输入(
            策略实现提交=输入.策略实现提交, 二进制哈希=输入.二进制哈希,
            参数={**输入.参数, "实验": "仅登录等待交易服务器同步", "不启动Tester": True},
            数据指纹=输入.数据指纹, 成本快照=输入.成本快照, 合约规格=输入.合约规格,
            mt5版本=输入.mt5版本, 建模方式=输入.建模方式, 起始日=输入.起始日, 结束日=输入.结束日, 分区=输入.分区,
        )
    编排器 = 中央实验编排器(
    ...
    结果 = 编排器.运行(
        输入,
        _仅登录执行器适配器(执行器) if 实参.仅登录等待同步 else 执行器,
    )
