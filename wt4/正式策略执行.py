...
def 运行BTC正式验收批次(配置: BTC正式批次运行配置) -> 正式策略验收批次结果:
    """运行真实 BTC 正式四期验收，严格固定准备、网络与执行顺序。

    在代理前置失败时函数在创建 Prefix 工件、编译或账本前终止，确保
    不会把离线状态错误记录成正式策略周期。运行期仍由单期执行器再次
    核验代理，防止准备和启动之间代理出口失效。
    """
    准备 = 配置.准备配置
    单期 = 配置.单期配置
    场景 = 配置.场景配置
    ...
    if not 配置.暂存根目录.parent.is_dir() or not 配置.工件根目录.parent.is_dir():
        raise ValueError("正式批次账本、暂存和工件根目录的父目录必须存在")
    准备结果 = 准备BTC正式验收批次(准备)
    编排器 = 中央实验编排器(
        追加式账本(配置.账本路径), 配置.暂存根目录, 配置.工件根目录
    )
    场景运行器 = 真实MT5正式场景运行器(场景)
    执行器 = tuple(正式BTC单期执行器(单期, 场景运行器) for _ in 准备结果.批次)
    return 运行正式策略验收批次(
        准备.截至日, 准备结果.批次, 编排器, 执行器
    )
