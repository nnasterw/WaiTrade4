...
def 核验离线代理隔离结果(日志证据: str, 期望代理地址: str | None = None) -> dict[str, object]:
    ...
    代理已命中 = bool(诊断["代理地址"]) and (
        期望代理地址 is None or 诊断["代理地址"] == 期望代理地址
    )
    未同步 = bool(生命周期["交易服务器未同步标记"])
    if 代理已命中 and 未同步:
        return {
            "通过": True,
            "结论": "离线代理已阻断MT5交易服务器同步",
            "原因": None,
        }
    缺失: list[str] = []
    if not 代理已命中:
        缺失.append("未观察到声明的不可用SOCKS5代理连接")
    if not 未同步:
        缺失.append("未观察到MT5交易服务器未同步标记")
    return {
        "通过": False,
        "结论": "离线代理隔离证据不足",
        "原因": "；".join(缺失),
...
def 核验离线仅登录隔离结果(日志证据: str, 期望代理地址: str) -> dict[str, object]:
    """核验仅登录反事实，不能错误要求不存在的 Tester 未同步标记。

    仅登录配置刻意没有 ``[Tester]``，因此不会生成
    ``not synchronized with trade server``。在外网已由 sandbox 阻断、
    代理地址又于启动前确认拒绝连接的前提下，本轮日志命中该不可用 SOCKS5
    且未出现授权或同步，只能证明本次受控运行未完成交易服务器授权。
    不把单条 ``connecting`` 日志解释为代理端已经拒绝连接：终端由本轮
    实验主动停止，MT5 不一定来得及写出连接失败记录。无直连回退由
    sandbox 的网络边界保证，而不是由此函数推断。
    """
    if not 期望代理地址:
        raise ValueError("离线仅登录核验必须声明代理地址")
    诊断 = 解析MT5代理同步诊断(日志证据)
    if 诊断["结论"] == "MT5交易服务器已同步" or 诊断["已授权服务器"]:
        return {
            "通过": False,
            "结论": "离线代理下仍出现MT5交易服务器授权或同步",
            "原因": "存在连接复用或直连回退风险，未实锤代理隔离",
        }
    if 诊断["代理地址"] != 期望代理地址:
        return {
...
class 单实例MT5探测执行器:
    ...
    def 执行(self, 输入: 实验输入, 暂存目录: Path) -> 执行结果:
        ...
        工件 = dict(结果.工件)
        # 隔离执行器在超时或非零退出时会保留后台 stdout/stderr、进程归属和
        # 执行日志，供能力边界结论审计。它们同样是本轮暂存目录中的真实
        # 工件；若不声明，归档器会正确拒绝存在未声明文件的归档。
        #
        # 不能无条件列入：执行器在启动前失败时可能尚未创建这些文件。
        for 名称 in ("后台-stdout.txt", "后台-stderr.txt", "后台-归属.json", "执行日志.txt"):
            路径 = 暂存目录 / 名称
        ...
        结果数据 = {
            **结果.结果,
            ...
            "MT5生命周期": 生命周期,
            "MT5代理同步诊断": 代理同步诊断,
            "MT5持久SOCKS5配置": [str(路径) for 路径 in 持久代理配置组],
        ...
        离线代理核验 = (
            核验离线代理隔离结果(日志文本, self.探测配置.代理地址)
            if self.离线代理隔离 else None
        )
        ...
        for 名称 in ("mt5-探测.ini", "mt5-启动.ini", *持久代理证据名称, "共享状态-运行前.json", "共享状态差异.json", 日志证据, *参数证据, *报告证据):
            ...
            工件[名称] = 隔离MT5执行器._哈希(路径)
        if not 报告证据:
            if self.离线代理隔离 and 离线代理核验 is not None and 离线代理核验["通过"]:
                return 执行结果(
                    实验状态.能力边界已验证,
                    工件,
                    {
                        **结果数据,
                        "能力边界": "不可用环回SOCKS5下，MT5未同步交易服务器且未形成回测报告",
                        "报告缺失为预期": True,
                    },
                )
            return 执行结果(
    ...
    def 执行仅登录同步(self, 输入: 实验输入, 暂存目录: Path) -> 执行结果:
        ...
        while monotonic() < 截止:
            ...
            if 诊断["结论"] == "MT5交易服务器已同步":
                已观察同步 = True
                break
            # 离线反事实不需要把整个等待窗口耗尽：日志命中声明的不可用
            # SOCKS5、且未完成授权即可提前收敛。此处只陈述本轮未完成
            # 授权；无直连回退由 sandbox 的网络边界保证。普通 7897
            # 登录探测仍严格等待至同步或超时。
            if self.离线代理隔离 and 核验离线仅登录隔离结果(
                临时日志, self.探测配置.代理地址
            )["通过"]:
                已观察离线阻断 = True
                break
        ...
        return 执行结果(
            ...
            工件,
            {
                ...
                "MT5代理同步诊断": 诊断,
                "离线代理隔离核验": (
                    核验离线仅登录隔离结果(日志文本, self.探测配置.代理地址)
                    if self.离线代理隔离 else None
                ),
